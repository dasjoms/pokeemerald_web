        ...
        for session in sessions.values_mut() {
            ...
            if let Some(active_walk) = session.active_walk_transition.as_mut() {
                ...
                if active_walk.is_complete() {
                    completed_walk_hop_landing_effect = active_walk.consume_hop_landing_effect();
            ...
            while let Some(input) = session.pop_walk_input() {
                ...
                if accepted {
                    ...
                    if should_latch_hop_landing_effect_on_transition_start(
                        ...
                        || (session.player_state.bike_runtime.queued_transition
                            == Some(BikeTransitionType::WheelieToNormal)
                            && session.player_state.bike_runtime.queued_transition_delay_ticks > 0)
                    {
                        if let Some(active_walk) = session.active_walk_transition.as_mut() {
...
fn should_latch_hop_landing_effect_on_transition_start(transition: BikeTransitionType) -> bool {
    matches!(
        transition,
...
fn update_bike_runtime_after_step(player_state: &mut PlayerState, direction: Direction) {
    match player_state.traversal_state {
        ...
        TraversalState::AcroBike => {
            ...
            apply_acro_runtime_outcome(player_state, Some(action), None);
        }
    }
}
...
fn update_bike_runtime_per_tick(
    ...
    let bunny_hop_cycle_tick_before_eval =
        matches!(player_state.bike_runtime.acro_runtime.state, AcroState::BunnyHop)
            .then_some(player_state.bike_runtime.acro_runtime.bunny_hop_cycle_tick());
    ...
    apply_acro_runtime_outcome(player_state, pending_action, bunny_hop_cycle_tick_before_eval);
}
...
fn apply_acro_runtime_outcome(
    ...
    bunny_hop_cycle_tick_before_eval: Option<u8>,
) {
    let should_defer_stationary_bunny_hop_setdown =
        matches!(action, Some(AcroAnimationAction::WheelieToNormal))
            ...
            && bunny_hop_cycle_tick_before_eval.unwrap_or_default() > 0
            && !player_state
                .bike_runtime
                .acro_runtime
                .hop_landed_this_tick();
...
fn flush_queued_acro_runtime_updates(player_state: &mut PlayerState) {
    ...
    apply_acro_runtime_outcome(player_state, None, None);
}
...
mod tests {
    ...
    #[tokio::test]
    async fn releasing_b_during_directional_hop_still_emits_one_landing_particle_at_action_completion(
    ) {
        let world = test_world_with_initial_map(MapData {
            map_id: "MAP_RELEASE_DURING_HOP_LANDING".to_string(),
            width: 4,
            height: 1,
            metatile_id: vec![0; 4],
            collision: vec![0; 4],
        ...
        let session = world
            ...
            .expect("session should create");
        world
            .join_session(session.connection_id, "release-during-active-hop-landing")
            .await
            .expect("session should join");
        let _ = drain_server_messages(&mut rx);

        {
            ...
            let session_state = sessions
                .get_mut(&session.connection_id)
                .expect("session should exist");
            session_state.player_state.map_id = "MAP_RELEASE_DURING_HOP_LANDING".to_string();
            session_state.player_state.tile_x = 0;
            session_state.player_state.tile_y = 0;
            session_state.player_state.facing = Direction::Right;
            session_state.player_state.traversal_state = TraversalState::AcroBike;
        ...
        assert!(
            ...
            "releasing B during the hop must still trigger WheelieToNormal at runtime boundary"
        );
        assert_eq!(
            landing_particle_count, 1,
    ...
    #[tokio::test]
    async fn bunny_hop_off_cadence_first_directional_input_waits_for_cadence_open() {
        let world = test_world_with_initial_map(MapData {
            map_id: "MAP_BUNNY_HOP_FIRST_DIRECTIONAL_CADENCE".to_string(),
            width: 8,
            height: 1,
            metatile_id: vec![0; 8],
            collision: vec![0; 8],
        ...
        let session = world
            ...
            .expect("session should create");
        world
            .join_session(session.connection_id, "bunny-hop-first-directional-cadence")
            .await
            .expect("session should join");
        let _ = drain_server_messages(&mut rx);
        ...
        world.tick().await;
        let first_tick_results: Vec<WalkResult> = drain_server_messages(&mut rx)
            .into_iter()
            .filter_map(|message| match message {
                ServerMessage::WalkResult(result) => Some(result),
                _ => None,
            ...
            .collect();
        assert!(
            first_tick_results.is_empty(),
            "first off-cadence directional bunny-hop input should be queued until cadence opens"
        );
        ...
        assert!(
            second_accepted.server_frame > first_accepted.server_frame,
            "second directional bunny-hop should continue after first accepted hop"
        );
    }
    ...
    #[test]
    fn releasing_b_while_in_bunny_hop_keeps_logical_substate_during_defer_window() {
        let mut player = test_player_state();
        player.traversal_state = TraversalState::AcroBike;
        player.bike_runtime.acro_runtime.state = AcroState::BunnyHop;
        player.bike_runtime.acro_runtime.bike_frame_counter = 40;
