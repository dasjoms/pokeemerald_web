...
describe("main movement pipeline integration", () => {
  ...
  it("keeps player screen-space coordinates stable while camera interpolation is active", () => {
    const TILE_SIZE = 16;
    const state: PipelineState = {
      playerTileX: 10,
      playerTileY: 7,
    ...
    const frameDeltaMs = stepDurationMs / 16;
    const playerScreenPositions: Array<{ x: number; y: number }> = [];
    ...
    while (activeWalkTransition !== null) {
      ...
      const rawDeltaX = (state.renderTileX - state.playerTileX) * TILE_SIZE;
      const rawDeltaY = (state.renderTileY - state.playerTileY) * TILE_SIZE;
      updateFieldCameraPixelOffset(cameraOffset, rawDeltaX, rawDeltaY, TILE_SIZE);

      const cameraCenterX =
        state.playerTileX * TILE_SIZE + TILE_SIZE / 2 + cameraOffset.xPixelOffset;
      const cameraCenterY =
        state.playerTileY * TILE_SIZE + TILE_SIZE / 2 + cameraOffset.yPixelOffset;
      const playerWorldX =
        state.playerTileX * TILE_SIZE + TILE_SIZE / 2 + cameraOffset.xPixelOffset;
      const playerWorldY =
        state.playerTileY * TILE_SIZE + TILE_SIZE + cameraOffset.yPixelOffset;

      playerScreenPositions.push({
        x: playerWorldX - cameraCenterX,
        y: playerWorldY - cameraCenterY,
      });
    }

    expect(playerScreenPositions.length).toBeGreaterThan(0);
    const baseline = playerScreenPositions[0];
    for (const sample of playerScreenPositions) {
      expect(sample.x).toBe(baseline.x);
      expect(sample.y).toBe(baseline.y);
    }
  });
