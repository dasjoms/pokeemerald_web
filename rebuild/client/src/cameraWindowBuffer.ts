...
export function resolvePreloadIncomingCameraSlice(
  origin: CameraWindowOrigin,
  offset: FieldCameraOffset,
  stepX: number,
  stepY: number,
  ...
  if (stepX < 0) {
    const bufferX = cameraMod(ringOffsetX - 1, CAMERA_METATILE_BUFFER_DIM);
  ...
  const bufferY = cameraMod(ringOffsetY - 1, CAMERA_METATILE_BUFFER_DIM);
